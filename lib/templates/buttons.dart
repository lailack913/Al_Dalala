...
class button {
  ...
  static filter(double height, double width, double fs,
      String type, bool isPressed,
      )
     {
    return
      Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(color: (isPressed == false ) ? Colors.grey : Colors.white, width: 3),
          ),
          child:
          Center(
            child:
            Text("$type",  style: TextStyle(
              fontSize: fs , color: (isPressed == false ) ? Colors.grey : Colors.white,
            ),),
          )
      );
  }
